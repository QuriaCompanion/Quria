...
class InventoryProvider with ChangeNotifier {
  ...
  Map<String, DestinyInventoryComponent> get characterInventory =>
      _characterInventory;
  Map<String, DestinyInventoryComponent> get characterEquipement =>
      _characterEquipment;
  ...
  List<DestinyItemComponent> getItemsByInstanceId(List<String?> ids) {
    ...
    items.addAll(
        _profileInventory.where((item) => ids.contains(item.itemInstanceId)));
    _characterEquipment.forEach((id, equipment) {
      if (equipment.items != null) {
        items.addAll(equipment.items!
            .where((item) => ids.contains(item.itemInstanceId)));
      }
    });
  ...
  int? getCurrentGrenadeHashForCharacter(
      BuildContext context, String characterId) {
  ...
  int? getCurrentMeleeHashForCharacter(
      BuildContext context, String characterId) {
  ...
  int? getCurrentSuperHashForCharacter(
      BuildContext context, String characterId) {
    DestinyItemComponent? subclass =
        getCurrentSubClassForCharacter(characterId);
    List<DestinyItemSocketState>? sockets =
  ...
  bool isItemEquipped(String itemInstanceId) {
    ...
    return _characterEquipment[owner]!
        .items!
  ...
  void setCharacterInventories(
      Map<String, DestinyInventoryComponent> characterInventory) {
    _characterInventory = characterInventory;
    notifyListeners();
  }

  void setCharacterEquipment(
      Map<String, DestinyInventoryComponent> characterEquipement) {
  ...
  void moveItem(String instanceId, String? newLocation, bool equip) {
    ...
    final previousItem = _characterEquipment[newLocation]?.items?.firstWhere(
        (element) =>
            ManifestService
                .manifestParsed
                .destinyInventoryItemDefinition[element.itemHash]
            ...
            ManifestService
                ...
                ?.equippingBlock
                ?.equipmentSlotTypeHash);
    final previousIndex = _characterEquipment[newLocation]?.items?.indexWhere(
        (element) =>
            ManifestService
                .manifestParsed
                .destinyInventoryItemDefinition[element.itemHash]
    ...
    if (newLocation == null) {
      ...
      _characterEquipment[newLocation]!
          .items!
